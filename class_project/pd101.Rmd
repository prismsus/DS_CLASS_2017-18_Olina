...
##Combine tables
- Combined tables from two seasons to one, added variable gender (F & M)
```{r}
...
```
t3 <- ggplot(df) +
  geom_freqpoly(mapping = aes(x = df$pd101_s1_trainee[[rlang::UQ(rlang::sym(x))]], color = Gender)) +
  scale_color_manual(values=c("#ff82ab", "#5cacee"))
t3 <- ggplotly(t3)
subplot(aa, t3, nrows = 2, margin = 0.05, heights = c(0.45, 0.55))
...
analyze_variable <- function(x){
  range <- max(df[[rlang::UQ(rlang::sym(x))]]) - min(df[[rlang::UQ(rlang::sym(x))]])
a <- df %>%
  count(cut_width(rlang::UQ(rlang::sym(x)), range/4))
b <- df %>%
  group_by(Gender) %>%
  summarise(mean_age = mean(rlang::UQ(rlang::sym(x)), na.rm = T))
list(a,b)
}
...
t3 <- ggplotly(t3)
tt <- subplot(t1, t2, shareY = TRUE)
subplot(tt, t3, nrows = 2, margin = 0.05, heights = c(0.45, 0.55))
}
```
