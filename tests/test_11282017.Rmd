...
```{r}
#x is the dataframe, y is the name for new column, ... is the formula
calc_customized_func <- function(x, y, ...) {
  x %>%
  mutate(y = eval(parse(text=...)))
}

#calc_customized_func(x,y,...)
```

```{r}
calc_customized_func(mtcars, y, "wt+qsec")
```
...
```{r}
mtcars %>%
  mutate(y = eval(parse(text="vs+am")))

```
```{r}
calc_customized_func_ <- function(x, y, ...) {
  df <- x %>%
  mutate(...)
...
calc_customized_func_ (mtcars, "hey", vs+am)
```
